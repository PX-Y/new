...
init_from = 'scratch'  # 'scratch' | 'resume' | 'gpt2' | 'gpt2-medium' | 'gpt2-large' | 'gpt2-xl'
...
# one-shot hard quantization after warmup
hard_quant_after_warmup = True
hard_quant_iter = 500  # -1 means use warmup_iters
...
@torch.no_grad()
def hard_quantize_model_inplace(model, n_bits_w, alpha_w,
                                include_substrings=None,
                                exclude_substrings=("ln_", "bias", "wte", "wpe"),
                                verbose=True):
    """
    One-shot hard quantization:
    snap all selected quantizable parameters to nearest quantization points once.
    If skip_saturated_fp=True and no explicit levels are used, saturated entries
    outside the clip range are left full precision.
    """
    backup = {}
    total = 0
    quantized = 0
    ...
    for name, p in _iter_named_quant_params(model, include_substrings, exclude_substrings):
        ...
        if skip_saturated_fp:
            # inside quantization range: snap to grid
            # outside quantization range: keep full precision
            w.copy_(torch.where(active_mask, q, w))
            quantized += int(active_mask.sum().item())
            skipped_sat += int(sat_mask.sum().item())
        else:
            w.copy_(q)
            quantized += int(w.numel())

        total += int(w.numel())

    if verbose:
        pct = 100.0 * quantized / max(total, 1)
        if skip_saturated_fp:
            print(
                f"[hard-quant-once] quantized {pct:.3f}% "
                f"({quantized:,}/{total:,}; skipped_sat={skipped_sat:,})"
            )
        else:
            print(f"[hard-quant-once] quantized {pct:.3f}% ({quantized:,}/{total:,})")
...
while True:
    ...
    # One-shot hard quantization after warmup.
    # This snaps selected weights once, then training continues with the original soft quant logic.
    hq_iter = warmup_iters if hard_quant_iter < 0 else hard_quant_iter
    if hard_quant_after_warmup and iter_num == hq_iter:
        if master_process:
            print(f"[hard-quant-once] iter={iter_num}, snapping selected weights once")
