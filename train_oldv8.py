...
def _quantize_to_grid_with_masks(w, n_bits, alpha_step):
    """Return quantized tensor and masks for non-saturated vs saturated entries.

    If skip_saturated_fp=True, entries with |w| > clip are treated as saturated:
    they are excluded from the quantization penalty and kept as pure FP updates.
    """
    L = (2 ** (n_bits - 1)) - 1
    step = alpha_step
    clip = L * step
